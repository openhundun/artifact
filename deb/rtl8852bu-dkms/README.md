...
RTL8852BU / RTL8832BU 无线网卡的 **DKMS 驱动 deb 包**：安装即编译，内核升级自动重编，Debian/Ubuntu 任意架构通用（`Architecture: all`，编译发生在目标机）。仓库自包含（固件随仓），`build.sh` 一条命令出包。

基于 [morrownr/rtl8852bu](https://github.com/morrownr/rtl8852bu) 上游驱动打包；固件取自 RTK 官方 Linux 驱动包（[RTK-AX1800_Linux_Driver_v1.1](https://download.lulian.cn/2026-drive/RTK-AX1800_Linux_Driver_v1.1.zip)）。
...
预编译 deb 发布在 [GitHub Releases](https://github.com/openhundun/artifact/releases)（文件名 `rtl8852bu-dkms_<版本>_all.deb`）。源码构建见下。
...
```bash
sudo apt install ./rtl8852bu-dkms_<版本>_all.deb
```

postinst 自动：清旧版本 DKMS 注册 → 遍历 `/lib/modules/*/build` 对**所有已装 headers 的内核**逐个 build+install（实测 `dkms autoinstall` 对非运行内核静默跳过，不可依赖）→ `depmod`。linux-headers 全缺时跳过构建，下次内核安装时由 dkms 钩子补编。插卡即用。
...
| 平台          | 内核                | 结果                                                                                  |
| ------------- | ------------------- | ------------------------------------------------------------------------------------- |
| Debian 13     | 6.12.101 + 6.12.105 | -5 安装即双内核 build/install ✓；modprobe 后接口生成，hostapd AP 实跑                 |
| Debian 13     | 6.12.101 + 6.12.105 | 同左（x86_64, 老 Celeron 平台各 ~6min）；modeswitch→b832，模块自载                    |
| Ubuntu 26.04  | 7.0.0-30-generic    | build/install ✓（验证上游 7.1 适配覆盖 7.0），modprobe ✓                              |
...
5. **USB3 模式默认不开**：`rtw_switch_usb_mode=1` 裸机可提 USB 吞吐，但部分虚拟化平台直通下丢设备；要开用 `modprobe 8852bu rtw_switch_usb_mode=1` 临时传参或自建 `/etc/modprobe.d/` 文件
...
上游驱动源码随包分发于 `/usr/src/rtl8852bu-20250826/`，版权归 Realtek / 上游维护者（见源码内 LICENSE）。
