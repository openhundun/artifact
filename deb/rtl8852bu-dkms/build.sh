...
OUT="${OUT:-${HERE}}"
SRC="${SRC:-}"
FIRMWARE="${FIRMWARE:-${HERE}/files/firmware/rtl8852bfw_rom.bin}"
BUILD="$(mktemp -d)"
...
dkms_ver="$(sed -n 's/^PACKAGE_VERSION="\(.*\)"/\1/p' "${HERE}/files/dkms.conf")"
...
cp "${HERE}/files/dkms.conf" "${ROOT}/usr/src/rtl8852bu-${VER}/dkms.conf"
cp "${FIRMWARE}" "${ROOT}/lib/firmware/rtl8852b/"
cp "${HERE}/files/usb-modeswitch.0bda-1a2b" "${ROOT}/etc/usb_modeswitch.d/0bda:1a2b"
