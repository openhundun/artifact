...
main() {
    ...
    check_env() {
        ...
        if [[ "${control_ver}" != "${VER}-${REV}" ]] || [[ "${dkms_ver}" != "${VER}" ]]; then
            die "version mismatch: control=${control_ver} dkms=${dkms_ver} expected ${VER}-${REV}/${VER}"
        fi
    }
    ...
    apply_patch() {
        ...
        grep -q 'n/a' "${BUILD}/src/core/rtw_debug.c" || die "patch not applied (upstream source changed?)"
    }
